...
info['pid_cam2'] = os.getpid()

UNIT = info['unit']
GPU_ID = info['gpu_id']
CAM_NUM = 2
OBJ_NAME = info['obj_name_cam2']
...
# THRESHOLD = 0.00522516
THRESHOLD = 0.15
MASK, BBOX = crop(MASK_CONFIG, 2)
...
class ad_models:
    def __init__(self):
        ...
        self.test_connect = pyjds.DeviceFactory.create(self.camera[1])
...
def image_preprocessing(image):
    ...
    template = cv2.imread('./template/b2.jpg', 0)
    template_top_left = (707, 213)
    ...
    cropped_image = result[ymin:ymax, xmin:xmax, :]

    img = cv2.resize(cropped_image.copy(), dsize=(512, 256))
    img = img / 255.0
...
def init_state(models):
    _, dummy_image = image_preprocessing(cv2.imread(DUMMY_IMG, cv2.IMREAD_COLOR))
...
def inference(RGB):
    ...
    with torch.no_grad():
        ...
        # ret_image = cv2.bitwise_and(ret_image, ret_image, mask=mask.astype(np.uint8))
        # ret_image = ret_image.astype(np.float32) / 255
        ...
        if  round(float(image_score), 8) >= THRESHOLD:  # In Error
            img_name = save_image(OBJ_NAME, RGB.copy(), cropped_image, out_mask_cv, 'F')  ##NAS에 이미지 저장하는 코드
            ...
            image_path = f"{nas_root_dir}/ormal/{OBJ_NAME}/{str(today)}/image/{img_name}.jpg"
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            logger.info(message)
            send_message_to_slack("----- [Unit 1] cam2 error -----\n" + message)
